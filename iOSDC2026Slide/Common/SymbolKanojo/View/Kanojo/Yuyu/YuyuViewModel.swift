...
class YuyuViewModel: KanojoViewModel {
    @Published private(set) var activeAction: ActionType? = nil

    let faceVM = YuyuFaceViewModel()
    let eyebrowsVM = YuyuEyebrowsViewModel()
    let eyesVM = YuyuEyesViewModel()
    let mouthVM = YuyuMouthViewModel()
    let costumeVM = YuyuCostumeViewModel()
    let faceMaskVM = YuyuFaceMaskViewModel()
    let accessoriesVM = YuyuAccessoriesViewModel()
    let emotionalEmissionVM = YuyuEmotionalEmissionViewModel()
    let gestureVM = YuyuGestureViewModel()

    lazy var parts: some KanojoPartsProvider = Yuyu(
...
private extension YuyuViewModel {
